...
void	am_init(t_cub *c)
{
	...
	// init_img(c, &am->m_wall, M_PX, M_PX);
	// init_img(c, &am->m_space, M_PX, M_PX);
	...
	if (!ft_fill_color_int(am->ceiling, c->map.c_color)
		...
		// || !ft_fill_color(am->m_space, (t_color){39, 44, 55, 255})
		// || !ft_fill_color(am->m_wall, (t_color){42, 46, 53, 255})
